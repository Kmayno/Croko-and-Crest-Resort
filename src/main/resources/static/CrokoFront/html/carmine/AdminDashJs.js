...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadAdmins() {
    ...
    fetch("http://localhost:8080/api/admin-login/getAllAdmin", {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject("Errore caricamento admins")
      )
  ...
  function renderAdmins(admins) {
    ...
    admins.forEach((admin) => {
      ...
      card.className =
        "bg-white shadow rounded-2xl p-6 hover:shadow-lg transition relative";
      card.innerHTML = `
        <h3 class="text-lg font-bold text-gray-800 mb-2">Admin: ${escapeHtml(
          admin.nome
        )}</h3>
        <p class="text-gray-600 mb-4"><span class="font-medium">Email:</span> ${escapeHtml(
          admin.email
        )}</p>
      ...
      card
        ...
        .addEventListener("click", () => openEditAdmin(admin, card));
      card
        .querySelector(".deleteBtn")
        .addEventListener("click", () => openDeleteAdmin(admin, card));
  ...
  document
    ...
    .addEventListener("click", closeEditAdmin);
  document
  ...
  document.getElementById("editAdminForm").addEventListener("submit", (e) => {
    ...
    fetch("http://localhost:8080/api/admin-login/update", {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject("Errore modifica admin")
      )
      .then(() => {
        closeEditAdmin();
        loadAdmins();
  ...
  function openDeleteAdmin(admin, card) {
    ...
    document.getElementById(
      "modalText"
    ).textContent = `Sei sicuro di voler eliminare ${admin.nome}?`;
  ...
  document
    ...
    .addEventListener("click", closeDeleteAdmin);
  document
  ...
  document.getElementById("confirmDelete").addEventListener("click", () => {
    ...
    fetch(
      `http://localhost:8080/api/admin-login/delete/${adminToDelete.idAdmin}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      .then((res) =>
        res.ok ? res : Promise.reject("Errore eliminazione admin")
      )
      .then(() => {
  ...
  document
    .getElementById("refreshPren")
    .addEventListener("click", loadPrenotazioni);

  function loadPrenotazioni() {
    // Carica prenotazioni confermate
    const prenDiv = document.getElementById("prenotazioniResult");
    prenDiv.innerHTML = `<div class="col-span-full text-gray-500">Caricamento prenotazioni confermate...</div>`;

    fetch("http://localhost:8080/api/prenotazioni?stato=true", {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
      ...
      .then((res) =>
        res.ok
          ? res.json()
          : Promise.reject("Errore caricamento prenotazioni confermate")
      )
      .then((pren) => renderPrenotazioni(pren, "confirmed"))
      .catch((err) => {
        console.error(err);
        renderPrenotazioni(samplePrenotazioni(), "confirmed");
        showBanner("Errore caricamento prenotazioni confermate", "red");
      });

    // Carica prenotazioni in attesa
    const pendingDiv = document.getElementById("pendingResult");
    pendingDiv.innerHTML = `<div class="col-span-full text-gray-500">Caricamento prenotazioni in attesa...</div>`;

    fetch("http://localhost:8080/api/prenotazioni/pending", {
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
      },
      ...
      .then((res) =>
        res.ok
          ? res.json()
          : Promise.reject("Errore caricamento prenotazioni in attesa")
      )
      .then((pren) => renderPrenotazioni(pren, "pending"))
      .catch((err) => {
        console.error(err);
        showBanner("Errore caricamento prenotazioni in attesa", "red");
      });
  }

  function renderPrenotazioni(prenotazioni, type = "confirmed") {
    const prenDiv =
      type === "pending"
        ? document.getElementById("pendingResult")
        : document.getElementById("prenotazioniResult");
    prenDiv.innerHTML = "";

    if (prenotazioni.length === 0) {
      prenDiv.innerHTML = `<div class="col-span-full text-gray-500 text-center py-8">Nessuna prenotazione ${
        type === "pending" ? "in attesa" : "confermata"
      }</div>`;
      return;
    }
    ...
    prenotazioni.forEach((pr) => {
      const card = document.createElement("div");
      card.className =
        "bg-white shadow rounded-2xl p-6 hover:shadow-lg transition";

      if (type === "pending") {
        // Card per prenotazioni in attesa
        card.innerHTML = `
                    <h3 class="text-lg font-bold text-yellow-700 mb-2">Prenotazione #${pr.idPrenotazione}</h3>
        ...
        card.innerHTML = `
                    <h3 class="text-lg font-bold text-green-700 mb-2">Prenotazione #${pr.idPrenotazione}</h3>
                    <p class="text-gray-600"><span class="font-medium">Nome cliente:</span> ${pr.nomeCliente}</p>
                    <p class="text-gray-600"><span class="font-medium">Check-in:</span> ${pr.dataInizio}</p>
                    <p class="text-gray-600"><span class="font-medium">Check-out:</span> ${pr.dataFine}</p>
                    <p class="text-gray-600"><span class="font-medium">Stanza:</span> ${pr.numeroStanza}</p>
                    <p class="text-gray-600"><span class="font-medium">Prezzo:</span> €${pr.prezzoTotale}</p>
                    <p class="text-green-600 font-medium">Confermata</p>
                ...
                `;
        card
          .querySelector(".deletePrenBtn")
          .addEventListener("click", () => openDeletePren(pr, card));
      }
      ...
      chk.addEventListener("change", async (e) => {
        ...
        try {
          const res = await fetch(
            `http://localhost:8080/api/prenotazioni/${id}?statoConfermato=${nuovoStato}`,
            {
              method: "PUT",
          ...
          showBanner(
            `Prenotazione #${id} aggiornata a ${
              nuovoStato ? "Confermata" : "Non confermata"
            }`,
            "green"
          );
  ...
  function openDeletePren(pren, card) {
    ...
    document.getElementById(
      "prenModalText"
    ).textContent = `Sei sicuro di voler eliminare la prenotazione #${pren.idPrenotazione}?`;
  ...
  document
    .getElementById("cancelPrenDelete")
  ...
  function openManageModal(pren) {
    ...
    detailsDiv.innerHTML = `
            <h4 class="font-bold text-lg mb-3">Dettagli Prenotazione #${pren.idPrenotazione}</h4>
            <p><strong>Cliente:</strong> ${pren.nomeCliente}</p>
            <p><strong>Check-in:</strong> ${pren.dataInizio}</p>
            <p><strong>Check-out:</strong> ${pren.dataFine}</p>
            <p><strong>Camera:</strong> ${pren.numeroStanza}</p>
            <p><strong>Prezzo:</strong> €${pren.prezzoTotale}</p>
        `;

    document.getElementById("approvePrenModal").classList.remove("hidden");
  }
  ...
  document.getElementById("showPendingBtn").addEventListener("click", () => {
    document.getElementById("pendingPrenotazioni").classList.remove("hidden");
    document.getElementById("confirmedPrenotazioni").classList.add("hidden");
  });

  document.getElementById("showConfirmedBtn").addEventListener("click", () => {
    document.getElementById("pendingPrenotazioni").classList.add("hidden");
    document.getElementById("confirmedPrenotazioni").classList.remove("hidden");
  });

  document.getElementById("confirmPrenDelete").addEventListener("click", () => {
    if (!prenToDelete) return;
    fetch(
      `http://localhost:8080/api/prenotazioni/${prenToDelete.idPrenotazione}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      .then((res) =>
        res.ok ? res : Promise.reject("Errore eliminazione prenotazione")
      )
      .then(() => {
  ...
  document
    .getElementById("refreshCamere")
    .addEventListener("click", loadCamere);
  ...
  function loadCamere() {
    ...
    fetch("http://localhost:8080/api/camere/getAllRooms", {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject("Errore caricamento camere")
      )
  ...
  function renderCamere(camere) {
    ...
    camere.forEach((c) => {
      ...
      card.className =
        "bg-white shadow rounded-2xl p-6 hover:shadow-lg transition";
      card.innerHTML = `
            <h3 class="text-lg font-bold text-gray-800 mb-2">Camera #${
              c.numeroStanza
            }</h3>
            <p class="text-gray-600"><span class="font-medium">Tipo:</span> ${
              c.tipoCamera || c.tipo
            }</p>
            <p class="text-gray-600"><span class="font-medium">Descrizione:</span> ${
              c.descrizione || ""
            }</p>
            <p class="text-gray-600"><span class="font-medium">Prezzo:</span> €${
              c.prezzoBaseNotte || c.prezzo || 0
            }</p>
            <p class="text-gray-600"><span class="font-medium">Disponibile:</span> ${
              c.disponibile ? "si" : "no"
            }</p>
      ...
      card
        ...
        .addEventListener("click", () => openEditCamera(c));
  ...
  function openEditCamera(cam) {
    ...
    document.getElementById("editTipoCamera").value =
      cam.tipo || cam.tipoCamera || "";
    document.getElementById("editDescrizione").value = cam.descrizione || "";
    document.getElementById("editPrezzo").value =
      cam.prezzoBaseNotte || cam.prezzo || 0;
  ...
  document
    .getElementById("cancelEditCamera")
    .addEventListener("click", closeEditCamera);
  document
    .getElementById("closeEditCameraModal")
    .addEventListener("click", closeEditCamera);
  ...
  document.getElementById("editCameraForm").addEventListener("submit", (e) => {
    ...
    fetch(`http://localhost:8080/api/camere/${cameraToEdit.idCamera}`, {
      ...
      .then((res) =>
        res.ok ? res.text() : Promise.reject("Errore modifica camera")
      )
      .then((msg) => {
        closeEditCamera();
  ...
  function openDeleteCamera(camera, card) {
    ...
    document.getElementById(
      "cameraModalText"
    ).textContent = `Sei sicuro di voler eliminare la camera #${camera.numeroStanza}?`;
  ...
  document
    .getElementById("cancelCameraDelete")
    .addEventListener("click", closeDeleteCamera);
  document
  ...
  document
    .getElementById("confirmCameraDelete")
    .addEventListener("click", async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8080/api/camere/${cameraToDelete.idCamera}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        closeDeleteCamera();
        showBanner(
          `Camera #${cameraToDelete.numeroStanza} eliminata con successo`,
          "green"
        );
      } catch (err) {
  ...
  document.getElementById("addCameraForm").addEventListener("submit", (e) => {
    ...
    const newCamera = {
      ...
      prezzoBaseNotte: parseFloat(
        document.getElementById("prezzoBaseNotte").value
      ),
  ...
  document.getElementById("addOpzioneForm").addEventListener("submit", (e) => {
    ...
    const newOpzione = {
      ...
      prezzoAggiuntivo: parseFloat(
        document.getElementById("prezzoAggiuntivo").value
      ),
    };
    ...
    fetch("http://localhost:8080/api/options/addOption", {
      ...
      .then((res) => {
        if (!res.ok)
          throw new Error("Errore durante l'inserimento dell'opzione");
  ...
  document
    .getElementById("refreshOpzioni")
    .addEventListener("click", loadOpzioni);
  ...
  function loadOpzioni() {
    ...
    fetch("http://localhost:8080/api/options/getAllOptions", {
      ...
      .then((res) =>
        res.ok ? res.json() : Promise.reject("Errore caricamento opzioni")
      )
...
`;
      ...
      card
        .querySelector(".deleteOpzioneBtn")
        .addEventListener("click", () => openDeleteOpzione(o, card));
      card
        .querySelector(".editOpzioneBtn")
        .addEventListener("click", () => openEditOpzione(o, card));
      resultDiv.appendChild(card);
    });
  }
  ...
  function openDeleteOpzione(opzione, card) {
    ...
    document.getElementById(
      "opzioneModalText"
    ).textContent = `Sei sicuro di voler eliminare l'opzione "${opzione.nomeOpzione}"?`;
  ...
  document
    .getElementById("cancelOpzioneDelete")
    .addEventListener("click", closeDeleteOpzione);
  document
    .getElementById("closeOpzioneModal")
    .addEventListener("click", closeDeleteOpzione);

  document
    .getElementById("confirmOpzioneDelete")
    .addEventListener("click", async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8080/api/options/delete/${opzioneToDelete.idOpzione}`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  function openEditOpzione(opzione, card) {
    ...
    document.getElementById("editPrezzoAggiuntivo").value =
      opzione.prezzoAggiuntivo;
  ...
  document
    .getElementById("cancelEditOpzione")
    .addEventListener("click", closeEditOpzione);
  document
    .getElementById("closeEditOpzioneModal")
    .addEventListener("click", closeEditOpzione);

  /////salvare modifica
  document
    .getElementById("editOpzioneForm")
    .addEventListener("submit", async (e) => {
      ...
      try {
        ...
        loadOpzioni();
        showBanner(
          `Opzione "${data.nomeOpzione}" aggiornata con successo`,
          "green"
        );
      } catch (err) {
